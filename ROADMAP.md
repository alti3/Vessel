...
| [x] | 10.09 | TLS | Implement certificate issuance | Provider-specific issuance is queued, audited, locked, and redacted | 10.08 | Traefik ACME issuance is lock-protected, audited with a durable Hangfire job id, and secrets are not emitted. |
| [x] | 10.10 | TLS | Implement certificate renewal job | Scheduled renewal is retry-safe, locked, and reports status | 5.25, 10.09 | `certificates.renew-due` is registered as a recurring Hangfire job; `CertificateRenewalJob` uses per-certificate locks and records renewal/failure state. |
